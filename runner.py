        ...
        try:
            while episode_num < self.params.n_episodes:
                ...
                if current_episode.is_finished:
                    ...
                    last_episodes_length = sum(len(ep) for ep in episodes[-100:]) / 100
                    pbar.set_description(f"{self.env.t.date().isoformat()} avg score={avg_score:.2f} - total={total:.2f},last_episodes_length={last_episodes_length}")
...
if __name__ == "__main__":
    ...
    params = Parameters(
        agent=PPOParameters.best_ppo() ,  #  VAEParameters.best_vae(), #PPOParameters.best_ppo() ,  #   #   #  #
        cardsim=CardSimParameters.paper_params(),
        clf_params=ClassificationParameters.paper_params(),
        n_episodes=10000,
        seed_value=42,
