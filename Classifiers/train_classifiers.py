...
def get_dataset_list(dataset_type: Literal["Generator", "Kaggle", "SkLearn"]):
...
def preprocess_dataset(fraud_fraction, normalize, dataset):
    ...
    balanced_dataset = pd.concat([fraud_data, non_fraud_sample]).sample(frac=1, random_state=42).reset_index(drop=True)
...
def fit_and_store_classifiers(fraud_fractions, classifier_types, classifier_classes, grids, dataset_type, normalize=True):
    """
    ...
    for dataset_name, global_dataset in datasets.items():
        process_datasets_and_train_clfs(
            dataset_name, global_dataset, fraud_fractions, classifier_types, classifier_classes, grids, dataset_type, normalize
        )


def process_datasets_and_train_clfs(
    dataset_name,
    global_dataset,
    fraud_fractions,
    classifier_types,
    ...
    for fraud_fraction in fraud_fractions:
        ...
        for classifier_type in classifier_types:
            train_and_save_classifier(
                classifier_type, classifier_classes[classifier_type], grids[classifier_type], train, val, dataset_base_path
            )
...
def _set_grids():
    grids = {
        "RF": {"n_estimators": [50, 100, 150], "max_depth": [None, 20], "min_samples_split": [2, 5, 10]},
        "BRF": {
