...
def add_frauds(customer_profiles_table, terminal_profiles_table, transactions_df):
    ...
    for day in range(transactions_df.TX_TIME_DAYS.max()):
        compromised_terminals = terminal_profiles_table.TERMINAL_ID.sample(n=3, random_state=day)
    ...
    # Scenario 3
    for day in range(transactions_df.TX_TIME_DAYS.max()):
        compromised_customers = customer_profiles_table.CUSTOMER_ID.sample(n=5, random_state=day).values
...
def generate_generator_dataset():
    ...
    transactions_df = transactions_df.groupby('TERMINAL_ID').\
        apply(lambda x: get_count_risk_rolling_window(x, delay_period=7, windows_size_in_days=[1,7,30],
                                                      feature="TERMINAL_ID"))
    ...
    transactions_df.drop(columns=['TRANSACTION_ID', 'TX_DATETIME', 'CUSTOMER_ID', 'TERMINAL_ID',
                                  'TX_TIME_SECONDS', 'TX_TIME_DAYS', 'TX_FRAUD_SCENARIO'], inplace=True)
    transactions_df.rename(columns={'TX_FRAUD': 'label'}, inplace=True)
    parent_dir = os.path.abspath('.')
    path_to_save = os.path.join(parent_dir, 'Dataset', 'Generator_Dataset', 'dataset.csv')
    transactions_df.to_csv(path_to_save, index=False)
