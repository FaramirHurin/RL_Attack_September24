...
def random_generator(batch_size, z_dim, T_mb, max_seq_len):
    ...
    for i in range(batch_size):
        ...
        temp_Z = np.random.uniform(0.0, 10, [T_mb[i], z_dim])
        temp[: T_mb[i], :] = temp_Z
        Z_mb.append(temp_Z)
    return Z_mb
...
def visualization(ori_data, generated_data, analysis):
    ...
    if analysis == "pca":
        ...
        plt.scatter(
            tsne_results[:anal_sample_no, 0], tsne_results[:anal_sample_no, 1], c=colors[:anal_sample_no], alpha=0.2, label="Original"
        )
        plt.scatter(
            tsne_results[anal_sample_no:, 0], tsne_results[anal_sample_no:, 1], c=colors[anal_sample_no:], alpha=0.2, label="Synthetic"
        )
...
def discriminative_score(num_batches, G, D, Supervisor, E, dataloader, batch_size, dim, seq_len, hidden_dim):
    """
    ...
    for i in range(num_batches):
        ...
        random_data = random_generator(batch_size=batch_size, z_dim=dim, T_mb=extract_time(x)[0], max_seq_len=extract_time(x)[1])
