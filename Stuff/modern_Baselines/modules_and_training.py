...
class Time_GAN_module(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.rnn_type in ["rnn", "gru"]:
            ...
            h0 = torch.zeros(self.n_layers, x.size(0), self.hidden_dim).to(self.device).float()
            c0 = torch.zeros(self.n_layers, x.size(0), self.hidden_dim).to(self.device).float()
            hidden = (h0, c0)
...
def TimeGAN(data, parameters):
    ...
    Discriminator = Time_GAN_module(
        input_size=hidden_dim, output_size=1, hidden_dim=hidden_dim, n_layers=num_layers, activation=nn.Identity
    )
    ...
    random_data = random_generator(
        batch_size=parameters["batch_size"], z_dim=dim, T_mb=extract_time(data)[0], max_seq_len=extract_time(data)[1]
    )
    ...
    for itt in range(epoch):
        ...
        for batch_index, X in enumerate(loader):
            ...
            print(
                ...
                + ", G_loss_U: "
                + str(G_loss_U.detach().numpy())
                + ", G_loss_S: "
                + str(G_loss_S.detach().numpy())
                + ", E_loss_t0: "
