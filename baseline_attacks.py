...
q_low = gan_transactions["amount"].quantile(0.05)
q_hi  = gan_transactions["amount"].quantile(0.95)
gan_transactions = gan_transactions[(gan_transactions["amount"] < q_hi) &
                                    (gan_transactions["amount"] > q_low)]

scaler = MinMaxScaler() #(-1,1) #unit_variance=True
...
atk_generator = Attack_Generation(device=device, criterion = nn.MSELoss(),
                              latent_dim=latent_dim, hidden_dim=hidden_dim, lr=0.001, trees=20)
atk_generator.train(real_data, batch_size=batch_size, num_epochs=num_epochs)

# Generate synthetic data
gen_data = atk_generator.generate_valid_samples(2000)
